...
const sms = (vima, m) => {
	...
	if (m.message) {
		...
		if (m.msg) {
			...
			if (m.quoted) {
				...
				m.quoted.delete = () => vimaa.sendMessage(m.chat, { delete: m.quoted.fakeObj.key })
				m.quoted.react = (emoji) => vimaa.sendMessage(m.chat, { react: { text: emoji, key: m.quoted.fakeObj.key } })
			}
		}
	...
	m.replyContact = (name, info, number) => {
		...
		vimaa.sendMessage(m.chat, { contacts: { displayName: name, contacts: [{ vcard }] } }, { quoted: m })
	}
